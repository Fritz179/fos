...
impl SDLPlatform {
    pub fn new(width: u32, height: u32) -> Box<SDLPlatform> {
        ...
        let window = video_subsystem.window("Salve!", width as u32, height as u32)
            .position_centered()
            .build()
    ...
    pub fn display_pixels(&mut self, tekenen: &Tekenen) {
        ...
        assert!(width * height * 4 == pixels.len() as u32, "Cannot render pixels!, Expected: {}, found: {}", width * height * 4, pixels.len());
    ...
    pub fn read_events(&mut self) -> Option<Event> {
        for event in self.event_pump.poll_iter() {
            match event {
                ...
                sdl2::event::Event::KeyDown { keymod, keycode, repeat, .. } => {
                    if let Some(keycode) = keycode {

                        let shift_mod: bool = keymod.bits() & (sdl2::keyboard::Mod::LSHIFTMOD.bits() | sdl2::keyboard::Mod::RSHIFTMOD.bits()) != 0;
                        let ctrl_mod: bool = keymod.bits() & (sdl2::keyboard::Mod::LCTRLMOD.bits() | sdl2::keyboard::Mod::RCTRLMOD.bits()) != 0;
